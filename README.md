...
3. Since both the server and React app want to use the default port of 3000, this will create a conflict. You therefore must create a .env file in the webportalclient directory, type in "PORT=3001" (or any other port), into that file so that the React app will use that port instead.
